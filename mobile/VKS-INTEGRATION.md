...
- **Мастер-VKS перехватывает у VK TURN**: при включённом мастере лестница
  стартует в любом режиме, и dialUpstream проверяет VKS-чейн ПЕРВЫМ (VK TURN
  остаётся резервом); при выключенном — как раньше. У каждого провайдера свой
  тумблер — выключенный выпадает из лестницы (можно оставить один для
