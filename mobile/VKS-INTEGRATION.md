...
- **Прямой зависимости клиента от сервера быть не должно.** Аудит (обновлён
  2026-09-24): в iOS-коде и вендоренном Go-дереве не должно оставаться ни IP,
  ни хостов оператора — при правках проверяйте это поиском по дереву. Рантайм-
  зависимостей нет.
...
- **VKS — только в whitelist-режиме**: лестница стартует, когда эффективный
  эндпоинт = whitelist И WhitelistMode = VKS. В Main-режиме лестница не
  стартует — основной H2 работает напрямую. Единый блок «Whitelist carrier»
  в настройках: пикер VKS / H2 / TURN (TURN — в конце); при выборе VKS внизу
  появляются тумблеры провайдеров (выключенный выпадает из лестницы — можно
  оставить один для изолированной проверки). В whitelist-режиме dialUpstream
  проверяет VKS-чейн ПЕРВЫМ (перехват у VK TURN, тот остаётся резервом).
