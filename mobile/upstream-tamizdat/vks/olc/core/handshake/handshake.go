...
import (
	...
	"github.com/funnybones69/tamizdat/vks/olc/core/framing"
)
...
func Client(rw io.ReadWriter, deviceID string, claims map[string]any) (string, string, error) {
	...
	log.Printf("[handshake] CLIENT_HELLO sent (%d claims, challenge %d chars)", len(hello.Claims), len(challenge))

	log.Printf("[handshake] waiting for welcome on stream...")
	for {
		sessionID, peerID, matched, replyErr := readReply(rw, challenge)
		log.Printf("[handshake] readReply: matched=%v sessionID=%q peerID=%q err=%v", matched, sessionID, peerID, replyErr)
		if !matched {
			continue
		}
...
func Server(rw io.ReadWriter, auth AuthFunc, peerID string) (Hello, string, error) {
	...
	log.Printf("[handshake] server got hello: len=%d", len(raw))
