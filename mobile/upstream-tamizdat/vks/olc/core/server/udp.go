...
import (
	...
	"github.com/funnybones69/tamizdat/vks/olc/core/tunnelcore"
	"github.com/funnybones69/tamizdat/vks/olc/core/udpwire"
)

// SOCKS5 UDP relay, server side: every datagram a peer sends is opened with
// the tunnel key, decoded as a udpwire frame and relayed to the target as a
// flow keyed by (peer, flow id); replies travel back sealed under the same
// key. Flow bytes are reported through OnTraffic like streams.
...
func (s *Server) handleDatagram(peerID string, ciphertext []byte) {
	...
	if peerID == "" && s.currentSessionID() == "" {
		// A datagram can only follow the handshake that opens the tunnel; a
		// broadcast before it has no session to attribute a flow to.
		logger.Debugf("drop udp datagram before session")
		return
	}
	wire, err := s.keys.OpenInto(nil, ciphertext, udpAAD())
	if err != nil {
...
func (s *Server) dialUDPFlow(target udpDialTarget) (net.Conn, error) {
	...
	dialer := &net.Dialer{Resolver: s.resolver}
...
// sendUDPFrame seals frame under the tunnel key and sends it back over the
// datagram lane.
func (s *Server) sendUDPFrame(peerID string, frame udpwire.Frame) bool {
	...
	enc, err := s.keys.SealInto(nil, wire, udpAAD())
	if err != nil {
...
func (s *Server) finishUDPFlow(flow *serverUDPFlow) {
	flow.closeOnce.Do(func() {
		_ = flow.conn.Close()
		if flow.sessionID == "" || s.onTraffic == nil {
			return
		}
		...
		// Attribute the flow's bytes to the session like a stream: up is
		// client to internet, which is bytesIn here.
