...
func (s *Server) dispatch(ctx context.Context, stream *smux.Stream, request ConnectRequest, sessionID string) {
	...
	started := time.Now()
	conn, err := s.dial(request, sessionID)
...
func (s *Server) dial(request ConnectRequest, sessionID string) (net.Conn, error) {
	if err := request.validate(); err != nil {
		return nil, err
	}
	addr := net.JoinHostPort(request.Addr, strconv.Itoa(request.Port))
	if s.dialHook != nil {
		return s.dialHook(context.Background(), addr, sessionID)
	}
